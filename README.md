...
**Windows 版 Mole** —— 参考 [Mole](https://github.com/tw93/Mole) 的设计理念，为 Windows 10/11 打造的深度清理、应用管理与系统监控图形工具。

单文件 PowerShell + WPF，无任何外部依赖。

> **定位说明**：Mole 官方在 macOS 上是免费开源 CLI（`mo`），图形界面版 Mole for Mac 为付费应用，其官方实验性 [windows 分支](https://github.com/tw93/Mole/tree/windows)同样是 CLI。Falco 填补的正是这个空缺——**Windows 上的 Mole 图形界面**：命令行的深度，图形界面的易用。

## 与 Mole 的模块对照

| Mole 模块 | Falco 对应 | 状态 |
|-----------|-------------|------|
| `status` 实时监控 + 健康度 | 状态页：八卡片 + 进程列表 | ✅ 已实现 |
| `optimize` 系统维护 | 优化页：三档一键优化 + 高级选项 | ✅ 已实现（Falco 超集，见下） |
| `clean` 深度清理 | 清理页：系统层 + 应用与开发缓存 | ✅ 已实现 |
| `uninstall` 应用卸载 | 软件页：启动项 + 应用卸载 + 残留清理 | ✅ 已实现（含深度残留扫描） |
| `analyze` 磁盘分析 | 分析页：目录占用（可下钻）+ 大文件扫描 | ✅ 已实现 |
| `purge` 构建产物清理 | 清理页：构建产物清理（按项目分组） | ✅ 已实现 |
| `installer` 安装包清理 | 清理页：安装包清理 | ✅ 已实现 |
| 菜单栏 HUD（Mole GUI） | 系统托盘 HUD | ✅ 已实现 |

P1–P6（Mole 功能对齐）与 V2-1 至 V2-4（优化引擎强化：修改验证、漂移检测、状态化单项管理、分维度健康度）**已全部实现**。
...
### 优化 — 三档一键优化 + 状态化单项管理（Falco 相对 Mole 的超集）
Mole 刻意不做系统调优，而这正是 Windows 用户的高频需求，因此保留：
...
> 🐣 **第一次使用？请阅读 [使用指南.md](./使用指南.md)**——面向新手的逐步说明：两种启动模式、五个页面各自怎么用、哪些操作放心做、出问题如何一键恢复。
